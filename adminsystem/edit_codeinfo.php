...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
        ...
        if (empty($_POST['code'])) {
		$errors[] = 'You forgot to enter code';
		} else {
		...
		$errors[] = 'You forgot to enter unitcost.';
		} else {
...
$q = "SELECT id,code,unitcost,description FROM dentalcode WHERE id=$id";
$result = @mysqli_query ($dbcon, $q);
...
{   // Valid user ID, display the form.
	...
	// Create the form
	echo '<form action="edit_codeinfo.php" method="post">
	<p><label class="label" for="name">Code:</label>
	<input class="fl-left" id="code" type="text" name="code" size="30" maxlength="30" 
	value="' . $row[1] . '"></p>
	
	<p><label class="label" for="unitcost">Unitcost:</label>
	<input class="fl-left" type="text" name="unitcost" size="30" maxlength="50" 
	value="' . $row[2] . '"></p>
